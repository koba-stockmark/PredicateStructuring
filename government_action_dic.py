class GovernmentActionRule:
    ...
    # 検討審議
    kentou_dic = [
        "審議", "判断", "可否", "審査", "諮問", "検討", "試算", "見直す", "策定", "継続審議と"
    ]
    # O-V 規則
    ...
    # O-V 規則
    o_v_sekou_dic = {"obj": {"措置", "施策", "対策", "対応策"},
                     "verb": {"実施", "行う"}
                     }
    ...
    kannkoku_dic = [
        ...
        "要請", "削除要請", "通知", "命令", "削除命令", "規制", "できない", "行政処分", "停止", "参加停止"
    ]
    ...
    # 意思決定(方針) , "始める", "開始", "まとめる", "定める", "強化", "決める", "決定","取りまとめる",
    decision_dic = [
    ...
    # 方策作成
    sakusei_dic = [
        "作る", "まとめる", "作成", "引き下げる", "引き上げる", "一つにする", "掲げる", "策定", "改正", "施行", "改訂"
    ]
    # O-V 規則
    o_v_sakusei_dic = {"obj": {"規定", "規則", "決まり", "法律", "対策", "漁獲枠", "枠"},
                       "verb": {"定める", "強化", "決める"}
                       }
    o_v_sakusei2_dic = {"obj": {"報告書", "レポート"},
                        "verb": {"取りまとめる"}
                        }
    ...
    # O-V 規則
    o_v_shien_dic = {"obj": {"補助金", "補助", "援助", "救済"},
                     "verb": {"下げる", "上げる", "増加", "削減", "減少", "打ち切る"}
                     }
    ...
    o_v_kaidan2_dic = {"obj": {"委員会", "会議", "議会", "会合"},
                       "verb": {"開催"}
                       }
...
#        "方針", "計画", "掲げる", "策定", "宣言"
    ...
    # 指数・統計
    shisuu_dic = [
        "回復", "上がる", "下がる", "増える", "増加", "減る", "減少", "上回る", "下回る", "引き上げる", "引き下げる", "割高",
        "1位", "2位", "3位", "4位", "5位", "6位", "7位", "8位", "9位",
        "1回", "2回", "3回", "4回", "5回", "6回", "7回", "8回", "9回"
    ]
    # O-V 規則
    o_v_shisuu_dic = {"obj": {"比率", "比", "率", "量", "％", "トン", "円", "ドル", "人", "額", "価"},
                      "verb": {"なる", "固まる", "値下がり", "値上がり", "高い", "安い", "超える", "上昇", "下降"}
                      }
    ...
    mokuhyou = ["<期日>", "<意思決定>", "<システム管理>", "<方針>"]
    houseibi = ["<検討>", "<施行>", "<改正>", "<廃止>", "<要請>", "<認可>", "<訓示>", "<訴訟>", "<勧告>", "<方策内容>", "<方策作成>"]
    hojyokin = ["<支援>", "<募集>", "<融資>", "<購入>", "<売却>"]
    zeisei = ["<財政>"]
    shisuu = ["<指数>"]
    kaigi = ["<会談>", "<派遣>"]
    sonota = ["<契約>", "<背景>", "<所見>", "<創立>", "<連携>", "<情報公開>", "<イベント>"]
