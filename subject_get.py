...
class SubjectExtractor:
    ...
    def rentai_check(self, pt , *doc):
        ...
        for cpt in range(pt + 1, doc[pt].head.i):
            if(doc[cpt].pos_ != 'AUX' and doc[cpt].pos_ != 'VERB'):
                break
    ...
    campany_special_vaerb = [
        ...
        '発行', '発信', '発売','発売', '発表', '販売', '販売開始', '務める', '目指す', '利用', '理念と', '立ち上げる'
    ]
    ...
    def subject_get_from_object(self, obj_point, *doc):
        ...
        # subjで直接接続をチェック
        for token in doc:
            if (token.dep_ == "nsubj" and (token.head.i == verb_pt and token.i != ng_pt and
                    token.tag_ != '名詞-普通名詞-副詞可能')):
