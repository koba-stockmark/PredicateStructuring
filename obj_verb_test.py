...
# 28悪影響
text = '内航海運の分野で、船員がかじを操作することなく航行できる無人運航船の実証実験が相次いで行われている。'
text = '伊那市福島の三澤寺は下諏訪町の模擬御柱の会から譲り受けた「模擬御柱」を活用して、本堂に安置する龍神像の制作に取り掛かる。'
text = '三菱電機のデータ連携基盤と連携させたスマートシティ/スーパーシティ関連システムの構築やデジタル地域通貨を用いた顔認証決済システムの実証実験を2022年度中に開始する予定としている。'
text = 'T&Tソリューションは、企業のサプライチェーンを管理し、偽造品に関連するリスクを軽減する機能を果たす。'
text = '横浜ゴムは、今年8月にタイヤ空気圧遠隔監視システムの実証実験を開始するなど、タイヤ市場や関連サービスの変化に対する取り組みを積極化している。'
